...
export default function WorkItemDetail({ title, role, year, linkUrl, modalContent, history, style }) {
    ...
    return (
        <Fragment>
            ...
            {sections.map((s, i) => (
                <DetailBodySection {...s} key={i} />
            ))}
            <Section altBg={true}>
                <h2 className={styles.toolsHeader}>Tools</h2>
                <div className={styles.toolsContainer}>
                    {tools.map((tool, i) => (
                        <ToolsIcon icon={tool} key={i} />
                    ))}
                </div>
            </Section>
            <Section>
...
class BackButton extends Component {
    render() {
        return (
            <button className={this.props.className} onClick={() => Router.back()}>
                <img src="/static/back_x.svg" alt="Back to my work" /> My Work
            </button>
        );
    }
}
